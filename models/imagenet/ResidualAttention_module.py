...
class AttentionModule_stage0(nn.Module):
    def __init__(self, in_channels, out_channels, size1=(112,112), size2=(56,56), size3=(28,28)):
    ...
    def forward(self, x):
        ...
        out_softmax2 = self.softmax2_blocks(out_mpool2)
        out_skip2_connection = self.skip2_connection_residual_block(out_softmax2)
        out_mpool3 = self.mpool3(out_mpool2)
        out_softmax3 = self.softmax3_blocks(out_mpool3)
...
class AttentionModule_stage2(nn.Module):
    def __init__(self, in_channles, out_channels, size1=(28,28), size2=(14,14)):
        super(AttentionModule_stage2, self).__init__()
        self.first_residual_blocks = ResidualBlock(in_channles, out_channels)
        self.trunk_branches = nn.Sequential(
            ResidualBlock(in_channles, out_channels),
            ResidualBlock(in_channles, out_channels)
        )

        self.mpool1 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.softmax1_blocks = ResidualBlock(in_channles, out_channels)
        self.skip1_connection_redisual_block = ResidualBlock(in_channles, out_channels)
        ...
        self.interpolation2 = nn.UpsamplingBilinear2d(size=size2)
        self.softmax3_blocks = ResidualBlock(in_channles, out_channels)
        self.interpolation1 = nn.UpsamplingBilinear2d(size=size1)
        ...
        self.last_blocks = ResidualBlock(in_channles, out_channels)
    ...
    def forward(self, x):
        ...
        out_mpool1 = self.mpool1(x)
        out_softmax1 = self.softmax1_blocks(out_mpool1)
        out_skip1_connection = self.skip1_connection_redisual_block(out_softmax1)
        out_mpool2 = self.mpool2(out_softmax1)
        out_softmax2 = self.softmax2_blocks(out_mpool2)
