...
def dpn131(num_classes=1000, pretrained=False, test_time_pool=True, output_feature=False):
    model = DPN(
        num_init_features=128, k_r=160, groups=40,
        k_sec=(4, 8, 28, 3), inc_sec=(16, 32, 32, 128),
        num_classes=num_classes, test_time_pool=test_time_pool, output_feature=output_feature)
    if pretrained:
        pretrained_dict = torch.load('./pretrained/dpn131-71dfe43e0.pth')
        model_state = model.state_dict()
        #print("%s\n" %(model_state.keys()))
        pretrained_state = {k:v for k,v in pretrained_dict.iteritems() if (k in model_state) and (k not in ['classifier.weight','classifier.bias'])}
        model_state.update(pretrained_state)
        model.load_state_dict(model_state) 
    return model

def dpn131_url(num_classes=1000, pretrained=False, test_time_pool=True):
    model = DPN(
        num_init_features=128, k_r=160, groups=40,
        k_sec=(4, 8, 28, 3), inc_sec=(16, 32, 32, 128),
        num_classes=num_classes, test_time_pool=test_time_pool)
...
class DPN(nn.Module):
    def __init__(self, small=False, num_init_features=64, k_r=96, groups=32,
                 ...
                 num_classes=1000, test_time_pool=False, output_feature=False):
    ...
    def forward(self, x):
        ...
        if not self.training and self.test_time_pool:
            ...
            for i in range(0, x.size(2)-7+1):
                for j in range(0, x.size(3)-7+1):
                    pool = F.avg_pool2d(x[:,:,i:i+7,j:j+7],kernel_size=7,stride=1)
                    #pool = pool.view(pool.size(0), -1)
                    xs.append(pool)
            feature = xs[0].view(xs[0].size(0), -1)
            out = torch.stack([self.classifier(x) for x in xs]).mean(dim=0)
        ...
        if self.output_feature:
            return out.view(out.size(0),-1), feature
        else:
