...
def main():
    ...
    testset = Dataset(root=args.root_val,
                      ...
                      transform=transforms.Compose([
                          ...
                          Lambda(lambda norms: torch.stack([normalize(norm) for norm in norms]))
                          ]))
    ...
    if args.output_feature:
        ...
        output_prob(val_loader, model, 0, use_cuda, args.save_dir,image_names,criterion)
    else:
...
def output_feature(val_loader, model, epoch, use_cuda, save_dir):
    ...
    fw = open(os.path.join(save_dir,'feature.txt'), 'w')
    fp = open(os.path.join(save_dir,'probability_tmp.txt'), 'w')
    for batch_idx, (inputs, targets, weights) in enumerate(val_loader):
        data_time.update(time.time() - end)
        ...
        for index in range(outputs.data.size(0)):
            ...
            for i in range(outputs.data.size(1)):
                feature += ' {}:{:.3f}'.format(i, output_feature[index][i])
            fw.write(feature)
            fw.write('\n')
...
def class_accuracy(targets, preds):
    topk = (1,5)
    maxk = max(topk)
    preds = preds[:, 0:maxk]
    res = []
    for k in topk:
        ...
        for idx, cls in enumerate(targets):
            topk_num[cls] += (pred_k[idx]==targets[idx]).sum()
            topk_sum[cls] += 1
        topk_acc = []
        for i in range(5000):
            topk_acc.append(topk_num[i]/(topk_sum[i]+0.0))
        res.append(np.mean(topk_acc))
    return res[0], res[1]

def output_prob(val_loader, model, epoch, use_cuda, save_dir, image_names, criterion):
    global avg_acc
    ...
    for batch_idx, (inputs, targets, weights) in enumerate(val_loader):
        ...
        bs, ncrops, c, h, w = inputs.size(0), inputs.size(1), inputs.size(2), inputs.size(3), inputs.size(4)
    ...
    df = pd.DataFrame(submit_result, columns=['image_id','predicted_0','predicted_1','predicted_2','predicted_3','predicted_4'])
    df.to_csv(os.path.join(save_dir, 'test_280.csv'),index=False,header=False,sep=' ')
    class_acc1, class_acc5 = class_accuracy(np.array(class_targets), np.array(class_preds))
    print('average per class accuracy: top1: {:.4f} top5: {:.4f}'.format(class_acc1, class_acc5))
    fw.close()
